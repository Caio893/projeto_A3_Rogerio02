...
3) Os .bat usam `.venv\Scripts\python.exe` e, se preciso, o runtime embed `python-3.10.11-embed-amd64\python.exe`. As dependencias ja estao em `.venv\Lib\site-packages` (inclui SDK FBX), entao nao precisa instalar Python.
...
- Rode manualmente:
  ...
  ```
- Se aparecer `ModuleNotFoundError: No module named 'fbx'`, instale o wheel `fbx-2020.3.7-cp310-none-win_amd64.whl` (veja "Recriar o ambiente").

Recriar o ambiente do zero (caso a .venv falhe ou falte no ZIP)
- Instale Python 3.10.11 64-bit com a opcao "Add python.exe to PATH".
...
- O repositório publico nao carrega a pasta `.venv` nem o wheel do SDK FBX (`fbx_sdk/.../fbx-2020.3.7-cp310-none-win_amd64.whl`). Sem eles, o Python embed nao tem o pacote `fbx`, entao o import falha logo no inicio.
- Os .bat foram ajustados para priorizar `.venv` e avisar quando ela estiver ausente, facilitando o diagnostico.
