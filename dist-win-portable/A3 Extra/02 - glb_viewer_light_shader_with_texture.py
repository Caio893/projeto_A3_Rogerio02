...
def compute_glb_model_bounding_box(glb_model):
    if len(glb_model) == 0:
        return None

    glb_model_bounding_box = compute_bounding_box(glb_model[0][0], glb_model[0][3])
   
    for i in range(1, len(glb_model)):
        mesh = glb_model[i]
        mesh_bounding_box = compute_bounding_box(mesh[0], mesh[3])

        glb_model_bounding_box = union_bounding_boxes(glb_model_bounding_box, mesh_bounding_box)

    return glb_model_bounding_box


def bind_texture(texture_image):
    if texture_image is None:
        return None

    texture_data = np.array(texture_image.getdata(), np.uint8)
...
def my_init(glb_file_path):
    glClearColor(1, 1, 1, 1)
    glEnable(GL_DEPTH_TEST)
    
    # carrega o modelo
    global glb_model
    glb_model = load_glb_model_with_texture(glb_file_path)
    bounding_box = compute_glb_model_bounding_box(glb_model)
    ...
    shader_gouraud_shading_program = create_shader_program('02 - glb_viewer_light_shader_with_texture_gouraud_vs.glsl', '02 - glb_viewer_light_shader_with_texture_gouraud_fs.glsl')

    global shader_phong_shading_program
    shader_phong_shading_program = create_shader_program('02 - glb_viewer_light_shader_with_texture_phong_vs.glsl', '02 - glb_viewer_light_shader_with_texture_phong_fs.glsl')
    ...
    for mesh in glb_model:
        vertices_pos, vertices_normals, vertices_uvs, faces, faces_normals, texture_image = mesh
        ...
        texture_id = bind_texture(texture_image)
